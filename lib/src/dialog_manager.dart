...
class DialogManager {
  ...
  static PopupDialogStyle defaultPopupDialogStyle = PopupDialogStyle(
    dialogStyle: const DialogStyle(
      dialogPlacement: DialogPlacement.top,
      margin: 0.0,
      elevation: 0,
      curvedRadius: 0,
    ),
  ...
  ///use this method to switch all dialogs to dark theme
  static void switchToDarkMode() {
    darkMode = true;
  }

  ///use this method to switch all dialogs to light theme
  ...
  /// Use this method to dismiss any message dialog using the optional [dialogId]
  static dismissMessageDialog({String? dialogId}) {
    MessageDialogController.instance
        .hideDialog(dialogKey: dialogId ?? DEFAULT_MESSAGE_DIALOG_ID);
  }
        ...
        MessageDialog(
          ...
          messageDialogStyle: MessageDialogStyle.empty(
              imageOrIconStyle: const ImageOrIconStyle(
                  size: 100,
                  imageOrIconPlacement: ImageOrIconPlacement.top,
                  color: Colors.blue)),
        )),
        transitionBuilder: getTransition(dialogEntrance: dialogEntrance));
  }
  ...
  /// use this method to show a list dialog
  static _showListDialog(
    ...
    launchNewScreen(
      ...
      List<ListItem> selectedListItems = List.generate(
          selectedIds.length,
          (index) => listItems
              .firstWhere((element) => element.itemKey == selectedIds[index]));
  ...
  static showSimpleListDialog(BuildContext context,
    ...
    ListDialogModel listDialogModel = ListDialogModel(
        listItems: listItems,
        title: title,
        listDialogStyle: listDialogStyle,
    ...
    _showListDialog(context,
        listDialogModel: listDialogModel, dialogEntrance: dialogEntrance,
        onItemSelected: (List<ListItem> resultItems) {
      if (maxSelections == 1) {
        ListItem listItem = resultItems[0];
        if (returnIndexes) {
          int index = listItems
              .indexWhere((element) => element.title == listItem.title);
        ...
        if (returnIndexes) {
          List itemIndexes = List.generate(resultItems.length,
              (index) => items.indexOf(resultItems[index].title));
          onItemSelected(itemIndexes);
        } else {
          List selectedItems = List.generate(
              resultItems.length, (index) => resultItems[index].title);
  ...
  static showComplexListDialog(BuildContext context,
    ...
    _showListDialog(context,
      ...
      if (maxSelections == 1) {
        ...
        if (returnIndexes) {
          int index = items
              .indexWhere((element) => element.itemKey == listItem.itemKey);
          onItemSelected(index);
        } else {
        ...
        if (returnIndexes) {
          List itemIndexes = List.generate(
              resultItems.length, (index) => items.indexOf(resultItems[index]));
          onItemSelected(itemIndexes);
        } else {
          List selectedItems =
              List.generate(resultItems.length, (index) => resultItems[index]);
          onItemSelected(selectedItems);
        }
      }
    });
  ...
  ///use this method to show an input dialog
  /// use [inputItems] to specify a list of inputs
  /// [onSummit] returns a list of String containing the data from the inputs
  /// the list is displayed in the order in which the data was requested
  ///
  static showInputDialog(BuildContext context,
    ...
    launchNewScreen(context, InputDialog(inputDialogModel: inputDialogModel),
        transitionBuilder: getTransition(dialogEntrance: dialogEntrance),
        result: (_) {
      if (_ == null) return;
      onSummit(_);
    ...
    launchNewScreen(
        ...
        PopupDialog(
            popupDialogModel: PopupDialogModel(
                ...
                popupDialogStyle: popupDialogStyle)),
        transitionBuilder: getTransition(
  ...
  static dynamic getTransition({DialogEntrance? dialogEntrance}) {
    ...
    return dialogEntrance == DialogEntrance.slide_left
        ? slideTransition
        : dialogEntrance == DialogEntrance.scale
            ? scaleTransition
            : dialogEntrance == DialogEntrance.fade_in
                ? fadeTransition
                : dialogEntrance == DialogEntrance.slide_down
                    ? slideDownTransition
                    : dialogEntrance == DialogEntrance.slide_right
